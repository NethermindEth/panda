"""Thin custom_ethnode wrappers over server operations.

Mirrors the ``ethnode`` module surface, but routes through the proxy's
``/custom/beacon`` and ``/custom/execution`` handlers. ``network`` and
``instance`` arguments must match a node configured under
``custom_ethnode.networks`` in the proxy config.
"""
...
def _require_custom_ethnode_available() -> None:
    if not os.environ.get("ETHPANDAOPS_CUSTOM_ETHNODE_AVAILABLE", "").strip():
        raise ValueError(
            "Custom ethnode is not enabled or no user-defined node endpoints are configured."
        )
...
def beacon_get(
    ...
    _require_custom_ethnode_available()
    payload = _runtime.invoke_json(
        "custom_ethnode.beacon_get",
        {
            "network": network,
            "instance": instance,
            "path": path,
...
def get_node_version(network: str, instance: str) -> dict[str, Any]:
    ...
    return _runtime.invoke_data(
        "custom_ethnode.get_node_version",
...
def get_node_syncing(network: str, instance: str) -> dict[str, Any]:
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.get_node_syncing",
        {"network": network, "instance": instance},
    )
...
def get_node_health(network: str, instance: str) -> int:
    ...
    data = _runtime.invoke_data(
        "custom_ethnode.get_node_health",
        {"network": network, "instance": instance},
    )
...
def get_beacon_headers(
    ...
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.get_beacon_headers",
...
def get_finality_checkpoints(
    ...
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.get_finality_checkpoints",
...
def get_config_spec(network: str, instance: str) -> dict[str, Any]:
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.get_config_spec",
        {"network": network, "instance": instance},
    )
...
def get_fork_schedule(network: str, instance: str) -> dict[str, Any]:
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.get_fork_schedule",
        {"network": network, "instance": instance},
    )
...
def get_deposit_contract(network: str, instance: str) -> dict[str, Any]:
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.get_deposit_contract",
        {"network": network, "instance": instance},
    )
...
def eth_block_number(network: str, instance: str) -> int:
    _require_custom_ethnode_available()
    data = _runtime.invoke_data(
        "custom_ethnode.eth_block_number",
        {"network": network, "instance": instance},
    )
...
def eth_get_block_by_number(
    ...
    _require_custom_ethnode_available()
    return _runtime.invoke_data(
        "custom_ethnode.eth_get_block_by_number",
        {
            "network": network,
            "instance": instance,
            "block": block,
...
def web3_client_version(network: str, instance: str) -> str:
    _require_custom_ethnode_available()
    data = _runtime.invoke_data(
        "custom_ethnode.web3_client_version",
        {"network": network, "instance": instance},
    )
